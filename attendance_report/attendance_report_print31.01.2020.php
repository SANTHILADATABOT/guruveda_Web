...
<table id="example" class="table table-striped table-bordered dt-responsive nowrap" cellspacing="0" width="850px" align="center">
    ...
    <tr>
        ...
        <td height="19" align="center" class="style7" style="font-size:14px;">ATTENDANCE REPORT </td>
        <td height="19" valign="top" align="right" class="style2">&nbsp;</td>
    </tr>
</table>
...
<table id="example" class="table table-striped table-bordered dt-responsive nowrap" cellspacing="0" width="850px" align="center">
    <thead>
        <tr>
            <td width="4%" height="27" align="left" class="top bottom style4"><strong>S.No</strong></td>
            <th width="15%"  align="left" class="top bottom style4">Staff name</th>
            <th width="15%"  align="right" class="top bottom style4">Actual Days</th>
            <th width="10%"  align="right" class="top bottom style4">T.W.Off</th>
            <th width="15%"  align="right" class="top bottom style4">Working Days</th>
            <th width="15%"  align="right" class="top bottom style4">Leave Taken</th>
...
//------------------------------------Finding sundays in month end --------------------------
    $days=cal_days_in_month(CAL_GREGORIAN,$month,$year);
 	$sql = "SELECT * FROM `user_creation` WHERE $get_query1 `delete_status`!='1' and user_type_id not in(1,2)";
	$rows = $db->fetch_all_array($sql);
	foreach($rows as $record)
	{
		$staff_name = $record['user_id'];
		$sql_late = "SELECT SEC_TO_TIME( SUM( TIME_TO_SEC( `late_hrs` ) ) ) as late_hrs FROM attendance_entry where staff_name='".$db->escape($staff_name)."' and DATE_FORMAT(entry_date,'%Y-%m') = '".$db->escape($from_month)."' and delete_status!='1' order by staff_name asc ";
		$sql_query2=$db->query_first($sql_late) ?? [];
		
		
		$sql_attendance = "SELECT * FROM  attendance_entry where staff_name='".$db->escape($staff_name)."' and DATE_FORMAT(entry_date,'%Y-%m') = '".$db->escape($from_month)."' and delete_status!='1'";
		...
		$late_hrs = $sql_query2['late_hrs'] ?? '';
		$late_hours = explode('.',$late_hrs);
		$actual_days =$days -  $num_sundays;
		$dateDiff = "";
		$sql_leave_count = "SELECT * FROM   leave_entry_form where staff_id='".$db->escape($staff_name)."' and DATE_FORMAT(leave_date,'%Y-%m') = '".$db->escape($from_month)."' and delete_status!='1'";
		$rs_leave = $db->query($sql_leave_count);
		$row_count = ($rs_leave && $rs_leave instanceof mysqli_result) ? $rs_leave->num_rows : 0;
		if($row_count==0)
		{
			$dateDiff=0;
		}
		...
		for($i=1;$i<=$days;$i++)
		{
			$new_date=$from_month."-".$i;
			$query = $db->query("SELECT * FROM   leave_entry_form where staff_id='".$db->escape($staff_name)."' and leave_date = '".$db->escape($new_date)."' and delete_status!='1'");
			while($sql_query4=$db->fetch_array($query))
			{
				
				$date1_ts = strtotime($sql_query4['from_date'] ?? '');
				$date2_ts = strtotime($sql_query4['to_date'] ?? '');
		...
		?>
			<tr>
            <td align="left" height="25" class="style2"><?php echo $j=$j+1;?></td>
            <td align="left" class="style2"><?php echo $record['staff_name'];?></td>
            <td align="right" class="style2"><?php echo $actual_days; ?></td>
            <td align="right" class="style2"><?php echo $num_sundays; ?></td>
            <td align="right" class="style2"><?php echo $working_days;?></td>
            <td align="right" class="style2"><?php echo $dateDiff;?></td>
            <td align="right" class="style2"><?php echo $late_hours[0] ;?></td>
        </tr>
         <?php 
